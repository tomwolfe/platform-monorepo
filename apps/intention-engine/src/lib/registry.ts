    ...
    execute:
      ...
      (async (params: any) => {
        if (!endpoint) {
          return {
            success: false,
            error: `No execution logic or endpoint provided for tool: ${name}`,
          };
        }
        ...
        try {
          ...
          if (!response.ok) {
            ...
            return {
              success: false,
              error: `External tool API error: ${errorText || response.statusText}`,
            };
          }
          ...
          return {
            success: false,
            error: `Failed to execute dynamic tool ${name}: ${errorMessage}`,
          };
        }
      }),
  };
