...
const logger = new Logger({ serviceName: "tool-calendar" });

export async function add_calendar_event(
  params: z.infer<typeof AddCalendarEventSchema>,
) {
  ...
  if (!validated.success) {
    ...
    if (singleEvent.success) {
      ...
      return {
        success: false,
        error: "Invalid parameters. Expected an array of events.",
      };
    }
  } else {
  ...
  logger.info("Adding calendar events", { eventCount: events.length });
  ...
  const serializedEvents = JSON.stringify(
    events.map((e) => ({
      ...
      description:
        e.restaurant_name || e.restaurant_address
