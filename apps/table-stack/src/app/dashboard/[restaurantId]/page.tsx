    ...
    <div className="p-8 max-w-7xl mx-auto">
      ...
      <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        ...
        <FloorPlan
          ...
          onSave={async (tables: { id: string, xPos: number | null, yPos: number | null }[]) => {
            'use server';
          ...
          onStatusChange={async (tableId: string, status: 'vacant' | 'occupied' | 'dirty') => {
            'use server';
          ...
          onUpdateDetails={async (tableId: string, details: { tableNumber: string, minCapacity: number, maxCapacity: number }) => {
            'use server';
      ...
      <DataTablesClient
        waitlist={restaurant.restaurantWaitlist.filter((w: any) => w.status !== 'seated')}
