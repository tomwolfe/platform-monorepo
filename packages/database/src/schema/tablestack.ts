...
export const restaurants = pgTable(
  "restaurants",
  {
    ...
    daysOpen: text("days_open").default(
...
export const restaurantReservations = pgTable(
  ...
  (table) => {
    return {
      ...
      restaurantIdIdx: index("reservations_restaurant_id_idx").on(
        table.restaurantId,
      ),
...
export const restaurantWaitlist = pgTable(
  ...
  (table) => {
    return {
      // Indexes for efficient querying
      restaurantIdIdx: index("waitlist_restaurant_id_idx").on(
        table.restaurantId,
      ),
      ...
      restaurantStatusCreatedAtIdx: index(
        "waitlist_restaurant_status_created_at_idx",
      ).on(table.restaurantId, table.status, table.createdAt),
    };
  },
);

export const restaurantProducts = pgTable(
  "restaurant_products",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    restaurantId: uuid("restaurant_id")
  ...
  (table) => {
    return {
      // Indexes for efficient querying
      restaurantIdIdx: index("products_restaurant_id_idx").on(
        table.restaurantId,
      ),
      categoryIdx: index("products_category_idx").on(table.category),
      // Composite index for querying products by restaurant and category
      restaurantCategoryIdx: index("products_restaurant_category_idx").on(
        table.restaurantId,
        table.category,
      ),
    };
...
export const inventoryLevels = pgTable(
  "inventory_levels",
  {
    ...
    productId: uuid("product_id")
      .references(() => restaurantProducts.id, { onDelete: "cascade" })
      .notNull(),
  ...
  (table) => {
    return {
      ...
      uniqueProduct: uniqueIndex("inventory_product_unique_idx").on(
        table.productId,
      ),
    };
  },
...
export const guestProfiles = pgTable(
  ...
  (table) => {
    return {
      restaurantEmailIdx: uniqueIndex("restaurant_email_idx").on(
        table.restaurantId,
        table.email,
      ),
    };
...
export const restaurantWaitlistRelations = relations(
  restaurantWaitlist,
  ({ one }) => ({
    restaurant: one(restaurants, {
...
export const restaurantProductsRelations = relations(
  restaurantProducts,
  ({ one, many }) => ({
    restaurant: one(restaurants, {
...
export const outbox = pgTable(
  "outbox",
  {
    ...
    // Execution ID for correlating events to a specific workflow execution
  ...
  (table) => {
    return {
      ...
      statusCreatedAtIdx: index("outbox_status_created_at_idx").on(
        table.status,
        table.createdAt,
      ),
 ...
 */
export const outboxDlq = pgTable(
  "outbox_dlq",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => {
    return {
      ...
      isRetriedCreatedAtIdx: index("outbox_dlq_is_retried_created_at_idx").on(
        table.isRetried,
        table.dlqCreatedAt,
      ),
      // Index for looking up DLQ events by execution ID
      executionIdIdx: index("outbox_dlq_execution_id_idx").on(
        table.executionId,
      ),
    };
  },
 ...
 */
export const processed_crypto_transactions = pgTable(
 ...
 */
export const crypto_transaction_speedups = pgTable(
  ...
  (table) => {
    return {
      ...
      replacementTxIdx: index("speedups_replacement_tx_idx").on(
        table.replacementTxHash,
      ),
    };
  },
);
...
export const drivers = pgTable(
  ...
  (table) => {
    return {
      ...
      locationIdx: index("drivers_location_idx").on(
        table.currentLat,
        table.currentLng,
      ),
      activeLocationIdx: index("drivers_active_location_idx").on(
        table.isActive,
        table.currentLat,
        table.currentLng,
      ),
...
export const orders = pgTable(
  "orders",
  {
    ...
    subtotal: numeric("subtotal", { precision: 78, scale: 0 })
      .notNull()
      .default("0"),
  ...
  (table) => {
    return {
      ...
      paymentTxHashIdx: uniqueIndex("orders_payment_tx_hash_idx").on(
        table.paymentTxHash,
      ),
      escrowStatusIdx: index("orders_escrow_status_idx").on(table.escrowStatus),
      payoutTxHashIdx: index("orders_payout_tx_hash_idx").on(
        table.payoutTxHash,
      ),
    };
  },
