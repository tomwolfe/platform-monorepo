...
describe("Structured Logger", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Reset global logger
    (globalThis as Record<string, unknown>).__globalLogger = undefined;
  });
  ...
  describe("Log Levels", () => {
    it("should log at DEBUG level", () => {
      ...
      expect((logs[0] as Record<string, unknown>).data).toBe("test");
    });
    ...
    it("should log error object with stack trace", () => {
      ...
      expect((logs[0] as Record<string, unknown>).code).toBe("DB_ERROR");
    });
  });
  ...
  describe("Query Logging", () => {
    ...
    it("should sanitize sensitive query data", () => {
      ...
      expect(logs[0].query).toContain("[PASSWORD_REDACTED]");
  ...
  describe("Performance Timing", () => {
    it("should start and end timer", () => {
      ...
      expect(logs.some((log) => log.message.includes("Timer completed"))).toBe(
        true,
      );
    });
  ...
  describe("Child Logger", () => {
    it("should create child logger with additional metadata", () => {
      ...
      const logs: LogEntry[] = [];
      (child as Record<string, (entry: LogEntry) => void>).output = (
        entry: LogEntry,
      ) => logs.push(entry);
  ...
  describe("withRequestLogging", () => {
    it("should wrap handler with request logging", async () => {
      ...
      expect(logs.some((log) => log.message === "Request completed")).toBe(
        true,
      );
    });
