...
export interface CacheOptions {
  /** Cache TTL in seconds (default: 300 = 5 minutes, or from env var) */
  ttlSeconds?: number;
  ...
  tags?: string[];
  /** Stale-while-revalidate duration in seconds (adds revalidation header) */
...
export function withCache<T extends (...args: any[]) => Promise<any>>(
  ...
  // Use provided TTL or determine from environment variables based on function name
  let ttlSeconds: number;
  if (options?.ttlSeconds !== undefined) {
    ttlSeconds = options.ttlSeconds;
  } else {
    // Environment-aware default TTLs
    const fnName = fn.name || "anonymous";
    if (
      fnName.includes("availability") ||
      fnName.toLowerCase().includes("availability")
    ) {
      ttlSeconds = parseInt(process.env.CACHE_TTL_AVAILABILITY || "30", 10);
    } else if (
      fnName.includes("restaurant") ||
      fnName.toLowerCase().includes("restaurant")
    ) {
      ttlSeconds = parseInt(process.env.CACHE_TTL_RESTAURANT || "300", 10);
    } else {
  ...
  return (async (...args: any[]) => {
    ...
    const cacheKey =
      options?.key ||
      generateCacheKey(fn.name || "anonymous", { args: stableStringify(args) });
    ...
    try {
      ...
      if (cached !== null) {
        ...
        if (result && typeof result === "object" && !result.headers) {
          result.cacheControl = buildCacheControlHeader(
            ttlSeconds,
...
function buildCacheControlHeader(
  ...
  return `public, max-age=${ttlSeconds}, stale-while-revalidate=${staleWhileRevalidate}`;
}
        ...
        return new Response(JSON.stringify(cached.body), {
          ...
          headers: {
            ...
            Age: Math.floor(
              (Date.now() -
                new Date(
                  cached.headers?.["cached-at"] || Date.now(),
                ).getTime()) /
                1000,
            ).toString(),
            ...cached.headers,
