...
const DANGEROUS_PATTERNS: Array<{
  ...
  // File system access
  {
    ...
    pattern:
      /\b(fs|path|file)\.(readFile|writeFile|appendFile|unlink|rm|mkdir|readdir|stat)\b/g,
    severity: "high",
  ...
  },
  {
    type: "PROTOTYPE_POLLUTION",
    pattern:
      /Object\.(defineProperty|setPrototypeOf|assign)\s*\(\s*Object\.prototype/g,
    severity: "critical",
  ...
  // Network access (SSRF)
  {
    ...
    pattern:
      /\b(http|https|net|dgram)\.(get|request|connect|createConnection)\b/g,
    severity: "medium",
...
// ============================================================================
// JSON SCHEMA TYPES (Subset used by MCP)
// ============================================================================

/**
 * Represents a JSON Schema property definition.
 * Models the subset of JSON Schema used by MCP tool input schemas.
 */
...
/**
 * Represents a complete JSON Schema for MCP tool input validation.
 */
...
export class MCPToolSecurityScanner {
  ...
  constructor(
    config?: Partial<typeof MCPToolSecurityScanner.prototype.config>,
  ) {
    this.config = {
      blockCritical: true,
      blockHigh: false,
  ...
  scanServer(server: MCPServerDefinition): SecurityScanResult {
    ...
    if (server.tools) {
      for (const tool of server.tools) {
        ...
        if (tool.inputSchema) {
          const schemaIssues = this.scanInputSchema(
            tool.inputSchema,
            tool.name,
          );
  ...
  scanCode(code: string, toolName?: string): SecurityIssue[] {
    ...
    // Check for denied patterns with validation
   ...
   */
  scanInputSchema(schema: InputSchema, toolName?: string): SecurityIssue[] {
    const issues: SecurityIssue[] = [];
  ...
  private generateRecommendations(issues: SecurityIssue[]): string[] {
    ...
    if (issueTypes.has("ENV_ACCESS")) {
      recommendations.add(
        "Consider using a secrets manager for sensitive configuration.",
      );
    }

    if (issueTypes.has("FILE_SYSTEM_ACCESS")) {
      recommendations.add(
        "Implement path validation and sandboxing for file operations.",
      );
    }

    if (
      issueTypes.has("NETWORK_ACCESS") ||
      issueTypes.has("SSRF_VULNERABILITY")
    ) {
      recommendations.add(
        "Implement URL allowlisting and block internal network access.",
      );
    }

    if (issueTypes.has("CODE_EXECUTION")) {
      recommendations.add(
        "CRITICAL: Remove all dynamic code execution. This is a severe security risk.",
      );
    }
  ...
  getSummary(result: SecurityScanResult): string {
    ...
    if (result.issues.length > 0) {
      ...
      parts.push(
        `Issues: ${bySeverity.critical} critical, ${bySeverity.high} high, ${bySeverity.medium} medium, ${bySeverity.low} low`,
      );
    }
...
export function createMCPToolSecurityScanner(
  config?: Partial<ConstructorParameters<typeof MCPToolSecurityScanner>[0]>,
): MCPToolSecurityScanner {
