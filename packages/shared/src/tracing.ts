...
export const ExecutionTraceEntrySchema = z.object({
  ...
  app_source: z.enum(['intention-engine', 'table-stack', 'open-delivery', 'shared']),
...
export function getTraceId(headers?: Headers | Record<string, string | string[] | undefined>): string {
  const store = tracingStorage.getStore();
...
export async function withNervousSystemTracing<T>(
  fn: (context: { correlationId: string; traceId: string; executionId?: string }) => Promise<T>,
  ...
  return tracingStorage.run({ correlationId, traceId, executionId }, () => fn({ correlationId, traceId, executionId }));
}
...
export function injectTracingHeaders(
  ...
  headers[CORRELATION_ID_HEADER] = correlationId || store?.correlationId || uuidv4();
  headers[TRACE_ID_HEADER] = traceId || store?.traceId || headers[CORRELATION_ID_HEADER];
...
export class InMemoryTraceEmitter implements TraceEmitter {
  ...
  async emit(entry: ExecutionTraceEntry): Promise<void> {
    ...
    console.log(`[Trace] ${entry.event_type} - ${entry.status} (${entry.latency_ms}ms)`);
  }
...
export function createTraceEntry(
  ...
  options?: Partial<Omit<ExecutionTraceEntry, 'trace_id' | 'execution_id' | 'event_type' | 'status' | 'timestamp'>>
): ExecutionTraceEntry {
  const store = tracingStorage.getStore();
