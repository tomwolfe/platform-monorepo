...
/**
 * ConnectWallet Component (Shared)
 *
 * T3.1: Consolidated Web3 UI Component
 * Single source of truth for wallet connection UI across all apps.
 *
 * Provides:
...
export function ConnectWallet() {
  ...
  return (
    <div className="relative">
      <details
        open={showDropdown}
        onToggle={(e) => setShowDropdown((e.target as HTMLDetailsElement).open)}
      >
        ...
        <div className="absolute right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border z-50 overflow-hidden">
          ...
          {balance && (
            <div className="p-3 border-b">
              ...
              {chain && (
                <p className="text-xs text-gray-500 mt-1">on {chain.name}</p>
              )}
            </div>
          )}
