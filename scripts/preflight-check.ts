...
const serviceChecks: ServiceCheck[] = [
  {
    ...
    check: async () => {
      ...
      const missingOptional = optional.filter(
        (v) => !process.env[v] && !process.env[optional[1]],
      );
...

  {
    ...
    check: async () => {
      try {
        ...
        const result = await QStashService.preflightCheck({
          throwOnError: false,
        });
    ...
    check: async () => {
      ...
      if (key.length < 64) {
        return {
          passed: false,
          error:
            "INTERNAL_SYSTEM_KEY must be at least 64 characters (32-byte hex string)",
        };
      }
      ...
      if (key === "internal-system-key-change-in-production") {
        if (process.env.NODE_ENV === "production") {
          return {
            passed: false,
            error:
              "Default INTERNAL_SYSTEM_KEY detected - change in production!",
          };
        }
        return {
          passed: true,
...
async function runPreflightChecks(serviceFilter?: string[]): Promise<boolean> {
  ...
  for (const service of serviceChecks) {
    ...
    if (
      serviceFilter &&
      !serviceFilter.some((f) =>
        service.name.toLowerCase().includes(f.toLowerCase()),
      )
    ) {
      continue;
    }
    ...
    if (result.passed) {
      logSuccess(
        `${service.name} (${elapsed}ms)${result.message ? ` - ${result.message}` : ""}`,
      );
  ...
  if (failedServices.length > 0 && process.env.NODE_ENV === "production") {
    logError(
      `CRITICAL: ${failedServices.length} service(s) failed pre-flight checks`,
    );
    logError(
      "Application cannot start in production mode with failed dependencies",
    );
    log("\nFailed services:");
    failedServices.forEach((s) =>
      log(`  - ${s.name}: ${s.error || "Unknown error"}`, colors.red),
    );
    log("\n");
    return false;
  }
  ...
  if (passedCount === totalCount) {
    ...
    logInfo(
      "Application may start with limited functionality in development mode",
    );
...
const servicesFilter = args.find((arg) => !arg.startsWith("--"))
  ? args
...
runPreflightChecks(servicesFilter)
  ...
  .catch((error) => {
    logError(
      `Pre-flight check failed: ${error instanceof Error ? error.message : String(error)}`,
    );
    process.exit(1);
  });
